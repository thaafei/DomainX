...
\subsection{Comparison Module}
  ...
  \subsection{Database Persistence Module}
    ...
    The rational for these tests are to ensure data is properly validated and stored, and validates the business logic and restrictions associated with the models.
    This is crucial for modules upstream which will depend on the data.
    An example set of such tests for the domain model are:
    \begin{enumerate}
      \item \texttt{test\_domain\_str\_returns\_name}: Tests if calling a domain object will return domain name instead of domain id.
      \item \texttt{test\_get\_domain\_id\_returns\_string\_uuid}: Tests that the get\_domain\_id function returns the uuid of the domain. 
      \item \texttt{test\_defaults\_and\_blank\_fields}: Tests the creation of a domain object with only required fields, and check if blank fields are populated with the expected values.
    \end{enumerate}

\subsection{Visualization Module}
  Since this module mainly consists of frontend components that calls the previously mentioned modules in order to render data for users, the following tests will be dynamic, since we have to run the code in order to test the output.
  \begin{enumerate}
    \item \texttt{test\_library\_raw\_values\_appear}: The raw values of the metric values should appear for the associated metrics across selected libraries. This tests that we are properly retrieving the correct information from the backend and database.
    \item \texttt{test\_missing\_metric\_values}: There should be user feedback when some libraries don't have the metric values filled for the selected metrics to visualize.
    \item \texttt{test\_error\_shown\_when\_less\_then\_two\_libraries\_are\_selected}: Comparing visualizations requires at minimum two libraries to be selected for a metric. There should be feedback if there are less than two.
    \item \texttt{test\_only\_display\_non\-text\_metrics}: There is no feasible way to display metrics that are text-based, we should filter these out so users can't select them.
    \item \texttt{test\_download\_visualizations}: Users should be able to successfully download all visualizations they currently have displayed.
  \end{enumerate} 
\section{References}
