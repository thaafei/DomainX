...
March 6, 2026  & Fei & Update Unit Test section\\
March 7, 2026  & Haniye & Updated Functional Requirement Plan Section\\
April 3, 2026 & Haniye, Ghena & Final documentation update contained listed in this \href{https://github.com/thaafei/DomainX/issues/538}{Github issue}.\\
\bottomrule
\end{tabularx}
\end{table}
...
Validation combined functional scenario testing, usability evaluation, and
non-functional performance checks as described below.

\subsubsection{End-to-End Validation}
Execute representative workflows, from data collection through AHP ranking and
visualization, to confirm system integrity. These tests were performed manually
by the research subteam using real library data.

\subsubsection{Usability Assessment}
Conducted with internal research users and Ryan Fang (Robotics domain student)
using short post-demo surveys (questions provided in Appendix~7.2). The success
criterion was a mean rating of $\geq 4.0$ (out of 5) across all survey questions.

\subsubsection{Performance Validation}
\label{subsubsec:performance-validation}

Performance validation ensured the system meets the speed and latency
requirements defined in the SRS (PR-SL1: AHP ranking calculation must complete
in less than ten seconds).

\textbf{Method:}
\begin{itemize}
  \item \textbf{Tools:} pytest with timing measurements, browser developer tools
  for front-end performance, and custom timing scripts for AHP calculations.
  
  \item \textbf{Timing:} Performance tests were conducted at two points:
  \begin{itemize}
    \item \textbf{After feature completion:} Basic response time checks for new
    endpoints to detect performance regressions early.
    \item \textbf{Before major releases (Rev 0 and Rev 1):} Full performance
    benchmark suite executed on representative datasets (5, 10, 20, and 40
    libraries).
  \end{itemize}
  
  \item \textbf{Metrics Collected:}
  \begin{itemize}
    \item Dashboard load time (median and 95th percentile)
    \item AHP recomputation time
    \item Filter application latency
    \item Export generation time
  \end{itemize}
\end{itemize}

\textbf{Pass/Fail Criteria and Remediation Plan:}
\begin{itemize}
  \item \textbf{Pass:} All measured response times are below the SRS thresholds
  (AHP $\leq$ 10 seconds; other actions $\leq$ 5 seconds for interactive use).
  
  \item \textbf{Fail:} Any response time exceeds the defined threshold.
  
  \item \textbf{Remediation Actions if Fail:}
  \begin{enumerate}
    \item Identify bottleneck using profiling tools (Django Debug Toolbar).
    \item Optimize database queries (add indexes, reduce N+1 queries).
    \item Implement caching for expensive calculations.
    \item Defer non-critical computations to background tasks (Celery).
    \item If threshold still not met after optimization, document the limitation
    and negotiate revised threshold with supervisor.
  \end{enumerate}
\end{itemize}

\subsubsection{Security Validation}
Attempt invalid inputs and authentication edge cases to ensure protection
against hazards such as data leakage and unauthorized access. This includes
SQL injection attempts (prevented by Django ORM), privilege escalation tests,
and invalid login attempts.

\textbf{Pass/Fail Criteria:}
\begin{itemize}
  \item \textbf{Pass:} All unauthorized actions return HTTP 403 Forbidden;
  audit logs capture all failed attempts; no data leakage occurs.
  \item \textbf{Fail:} Any unauthorized action succeeds or sensitive data is exposed.
  \item \textbf{Remediation:} Immediate patch of security vulnerability and
  re-validation before next release.
\end{itemize}

\subsubsection{Stakeholder and User Validation}
Regular validation was conducted through:
\begin{itemize}
  \item \textbf{Weekly stakeholder meetings:} Dr.~Spencer Smith reviewed
  implemented features and provided feedback on alignment with requirements.
  
  \item \textbf{User testing sessions:} The research subteam and Ryan Fang
  (Robotics domain student) used the tool and provided feedback on usability,
  missing features, and pain points. This feedback was tracked via GitHub
  issues and prioritized for subsequent sprints.
\end{itemize}

\subsection{Timeline and Iterative Process}
\label{subsec:timeline}

The V\&V activities followed an iterative process aligned with the team's agile development methodology. The timeline for verification and validation was structured as follows:

\begin{table}[H]
\centering
\caption{V\&V Timeline and Iterative Process}
\label{tab:vnv-timeline}
\begin{tabular}{|p{4cm}|p{10cm}|}
\hline
\textbf{Activity} & \textbf{Frequency / Timing} \\
\hline
Build Feature & Continuous, each sprint (2-week cycles) \\
\hline
Unit Testing & Immediately after feature completion, automated via CI/CD (pytest) \\
\hline
Integration Testing & After each sprint, before merging to main branch \\
\hline
Stakeholder Meeting & Weekly with Dr.~Spencer Smith to review progress and verify requirements \\
\hline
User Feedback Session & Conducted with research subteam and Ryan Fang (Robotics domain student) \\
\hline
Dev Team Meeting & Weekly to review progress, assign tasks, and adjust priorities \\
\hline
VnV Plan Update & After each feedback session, as needed \\
\hline
Regression Testing & Before each release (Rev 0, Rev 1) \\
\hline
\end{tabular}
\end{table}

\textbf{Iterative Process Description:}
\begin{enumerate}
  \item \textbf{Build Feature:} The development team implemented a new feature according to the sprint plan.
  
  \item \textbf{Unit Testing:} Automated unit tests were written using pytest and executed via GitHub Actions CI/CD pipeline. Features without passing unit tests could not be merged.
  
  \item \textbf{Integration Testing:} After feature completion, integration tests verified that the new feature worked correctly with existing system components.
  
  \item \textbf{Weekly Dev Team Meeting:} The team reviewed test results, identified failing tests, and prioritized fixes. Adjustments to the VnV plan were discussed based on observed outcomes.
  
  \item \textbf{Weekly Stakeholder Meeting:} Dr.~Smith reviewed progress, provided feedback on feature implementation and test coverage, and identified any gaps in requirements verification. All changes were tracked through GitHub issues.
  
  \item \textbf{User Feedback Session:} The research subteam and Ryan Fang (a Master's student who will use the tool for the Robotics domain) used the tool and provided feedback on usability, correctness, and missing features. This feedback directly informed updates to both the feature backlog and the VnV plan.
  
  \item \textbf{VnV Plan Update:} Based on feedback from testing, stakeholder review, and user sessions, the VnV plan was updated to reflect new test cases, modified success criteria, or changed priorities.
  
  \item \textbf{Regression Testing:} Before major releases (Rev 0 and Rev 1), the full test suite was executed to ensure no existing functionality was broken.
\end{enumerate}

This iterative approach ensured that verification and validation were continuous activities rather than end-of-cycle events, allowing for early detection of issues and incorporation of stakeholder feedback throughout development.
...
\textbf{Objective:} Verify that user account creation, login, role-based access control, and authentication operate correctly and protect system integrity.
...
\textbf{T--AC--2} &
Login as a Contributor and attempt to edit metric values. &
User can view and edit metric values through the interactive table. &
\textbf{SR--AC2} \\
\hline

\textbf{T--AC--3} &
Login as a Super Admin and access administrative functionality. &
User can access user management features and manage accounts. &
\textbf{SR--AC2} \\
\hline

\textbf{T--AC--4} &
Attempt login with invalid credentials or unregistered email. &
Access is denied; error message displayed; no session created. &
\textbf{SR--AC5, UH--UP3, LF--AR6} \\
\hline

\textbf{T--AC--5} &
Update user role (e.g., Contributor to Super Admin). &
Role is updated successfully and permissions reflect the change. &
\textbf{SR--AC4, PR--SC3, FR7} \\
\hline

\textbf{T--AC--6} &
Authenticated user updates password through profile page. &
Password is updated and required for future logins. &
\textbf{SR--AC5, PR--SC1} \\
\hline

\textbf{T--AC--7} &
Access system as a public or unauthorized user and attempt modification. &
Published data is viewable but modification actions are restricted. &
\textbf{SR--AC1} \\
\hline

\textbf{T--AC--8} &
Super Admin creates user and triggers password setup email. &
User receives email, sets password, and account becomes active. &
\textbf{FR2} \\
\hline

\textbf{T--AC--9} &
Attempt to access restricted functionality without sufficient privileges. &
Restricted actions are hidden and direct access is blocked or redirected. &
\textbf{SR--AC6, SR--IM1} \\
\hline

\textbf{T--AC--10} &
Perform password reset using email token. &
Reset link is valid for one use and password is updated securely. &
\textbf{FR4} \\
\hline
...
Black-box UI tests (\texttt{\href{https://www.selenium.dev/}{Selenium}}/\texttt{\href{https://playwright.dev/}{Playwright}}) will simulate login, role-based access, and password workflows.\\
Manual security validation will include privilege-restriction checks and unauthorized access attempts, referencing hazards identified in the Hazard Analysis (e.g., improper access control, credential misuse).\\[0.5em]
...
\textbf{T--DM--1} &
Create a new domain. &
Domain is stored in database with unique identifier and displayed in UI. &
\textbf{FR5, SR--INT1, SR--AC3} \\
\hline

\textbf{T--DM--2} &
Add library using GitHub repository URL. &
Repository metadata (commits, issues, LOC) is retrieved successfully via external tools. &
\textbf{FR7, FR10, OE--IA1, SR--INT2, SR--INT3} \\
\hline

\textbf{T--DM--3} &
Edit metric values for a library. &
Updated values are stored and AHP ranking is recalculated accordingly. &
\textbf{FR7, FR12, SR--INT4, SR--IM2} \\
\hline

\textbf{T--DM--4} &
Simulate API failure during repository analysis. &
System reports failure and preserves previously stored metric data. &
\textbf{PR--RFT1, PR--RFT2, SR--INT5} \\
\hline

\textbf{T--DM--5} &
Export domain data as Excel file. &
Downloaded file contains complete and correct domain and metric data. &
\textbf{FR14, SR--INT7, OE--PR4} \\
\hline

\textbf{T--DM--6} &
Publish a completed domain. &
Domain becomes publicly accessible for viewing and download. &
...
\noindent\textbf{Verification Approach:}\\
Dynamic system tests will validate database persistence, API integration, and data processing.\\
Automated UI tests will confirm front-end updates and form validation.\\
Failure scenarios (e.g., API timeout, missing data) will be tested to ensure robustness and data integrity, referencing the Hazard Analysis.\\
Export and publishing workflows will be verified through integration testing and file validation.\\[0.5em]


\subsubsection{Area of Testing 3: Visualization and Reporting}
\textbf{Objective:} Verify that visualization components and export functionalities correctly display and summarize analyzed data.
...
\textbf{T--VR--1} & Generate visualization for selected libraries and metrics. & Comparative graphs are displayed correctly and reflect stored values. & Auto & \textbf{FR13, FR12, SR--VIS1, SR--IM1} \\
\hline

\textbf{T--VR--2} & Modify selected metrics or libraries and regenerate graphs. & Graphs update dynamically to reflect new selections. & Auto & \textbf{FR13, SR--VIS2, PR--SL1} \\
\hline

\textbf{T--VR--3} & Download generated visualizations. & Visualization images are exported successfully (PNG). & Manual & \textbf{FR15, OE--PR4, SR--INT7} \\
\hline

\textbf{T--VR--4} & Resize browser or test on different screen resolutions. & Graphs remain readable and layout adapts correctly. & Manual & \textbf{LF--AR2, LF--SR2, UH--AR1} \\
\hline

\textbf{T--VR--5} & Generate visualization with missing metric values. & System handles missing data without runtime errors. & Auto & \textbf{PR--RFT1, SR--INT5} \\
\hline

\textbf{T--VR--6} & Open Manage Metrics page. & All metrics and definitions are displayed correctly. & Manual & \textbf{FR11, UH--LR2, LF--AR3} \\
\hline
...
\begin{itemize}
  \item \textbf{Dynamic Tests:} Executed using \texttt{\href{https://docs.pytest.org/en/stable/}{pytest}} for backend validation and \href{https://playwright.dev/}{Playwright} or \href{https://www.selenium.dev/}{Selenium} for UI verification.
  \item \textbf{Static Verification:} Manual inspection will confirm readability, layout consistency, and labeling accuracy.
  \item \textbf{Cross-Validation:} Visualization outputs will be compared against stored metric values and AHP rankings.
  \item \textbf{Failure Handling:} System behavior under missing data and API failures will be validated against Hazard Analysis controls.
\end{itemize}
...
\begin{itemize}
  \item Perform a code review of authentication modules to confirm proper password hashing (e.g., Django built-in hashing) and secure session-token handling.
  \item Attempt unauthorized actions (e.g., deleting data without sufficient privileges, accessing admin-only routes) and verify they are blocked (e.g., HTTP 401/403 responses) and do not modify system state.
  \item Review logging configuration to ensure security-relevant events (e.g., failed authentication attempts) are recorded with timestamps and associated user/context where applicable.
  \item Verify that secure cookie and HTTPS settings are enforced in production configuration.
\end{itemize}

\textbf{Expected Output:} Completed security-verification checklist, screenshots/logs of simulated unauthorized access attempts, and confirmation that restricted actions are blocked with appropriate responses.
...
We use \href{https://docs.pytest.org/en/stable/}{pytest} to implement unit tests for the system. All tests are done automatically through github actions, unless otherwise stated.
Unit tests will be located in the \href{https://github.com/thaafei/DomainX/tree/main/test}{test} subfolder of the project. More detailed reports of the unit tests can be found in the \href{https://github.com/thaafei/DomainX/blob/main/docs/VnVReport/VnVReport.pdf}{VnVReport} under the Unit Test section.

\subsection{Unit Testing Scope}
The scope of the unit tests will only be focused on modules developed by the DomainX team, which are:
\begin{multicols}{2}
  \begin{enumerate}
    \item Domains Page Module
    \item Data Edit Module
    \item User Management Module
    \item Automated Metrics Module
    \item Comparison Module
    \item Visualization Module
    \item Database Persistence Module
  \end{enumerate}
\end{multicols}

The \href{https://github.com/thaafei/DomainX/blob/main/docs/Design/SoftDetailedDes/MIS.pdf}{WSS} lists out all the modules and their use, syntax, and semantics.
Our testing philosophy is to consider the general use case of the user, with the most expected function. We will also include a few use cases where the user is expected to encounter errors.

\subsection{Domains Page Module}
  Tests for this module concerns the creation of a new domain, updating domain information and associated admins as creators to the domain. The test should provide assurance that mapping between creators and domains are working correctly, and business restrictions for creating new domains.
  \begin{enumerate}
    \item \texttt{test\_create\_domain\_success}: The base case, creating a new domain with domain name and description should be successful and the newly created domain should be stored in the database.
    \item \texttt{test\_create\_domain\_missing\_domain\_name/description}: We required domains to have both a name and description, if a domain is being created without one or the other, an error response should be returned and won't be created in the database.
    \item \texttt{test\_list/delete\_domains}: Basic functionalities such as listing all the domains and deleting a specific domain is working. Also testing that when deleting a domain, the mapping between creators and domains are destroyed.
    \item \texttt{test\_update\_domain\_sets\_creators}: The mapping between admin(creators) and domains is crucial so researchers can be associated with the data gathered. We test that this connection can be created successfully, and existing domains can edit this connection.
  \end{enumerate}

\subsection{Data Edit Module}
  Tests for this module checks that data is being stored correctly in the backend and that our validation and filtering mechanisms are working correctly so that we catch incorrect payloads.
  \begin{enumerate}
    \item \texttt{test\_list\_libraries\_returns\_only\_domain\_libraries}: Test if the API endpoint \texttt{/api/libraries/by\_domain/\{domain\_ID\}/} returns only the libraries belonging to the specific domain. As different domains can contain the same library, this function validates that it will return the correct associated data.
    \item \texttt{test\_create\_library\_requires\_domain}: API endpoint \texttt{/api/libraries/} to libraries can only create a new library if it is associated with a domain.
    \item \texttt{test\_create\_library\_invalid\_domain}: If a request to create a library is sent, there should be validations in place that the associated domain actually exists.
    \item \texttt{test\_create\_library\_success\_sets\_pending\_and\_returns\_task\_id}: When a new library is created, it automatically start an analysis of automated metrics.
    \item \texttt{test\_create\_library\_enqueue\_raises\_marks\_failed\_and\_task\_id\_none}: There should be proper user feedback for a created library that has a failed analysis.
    \item \texttt{test\_patch/put\_library\_success}: Updating single or multiple field related to the library should provide user feedback and changes should be made in the database.
    \item \texttt{test\_put\_library\_rejects\_non\_github\_url}: The tool depends on retrieving automated metrics from github, we test if a new library creation will be rejected if provided github url isn't actually from github.
    \item \texttt{test\_delete\_library\_success/fail}: Test both cases of deleting a library successfully and when deletion fails, such as provided library does not exist.
    \item \texttt{test\_library\_metric\_value\_update\_requires\_metrics\_dict}: Test the API endpoint of updating the associated metric values for a library will validate the payload and provide user feedback if it doesn't.
    \item \texttt{test\_library\_metric\_value\_update\_creates\_value\_and\_evidence}: Since we depend on external APIs to populate some of the metrics, we should have tests to ensure the proper filtering is in place. This test will check if given a payload, request to update an existing metric is successful, and request to update a nonexistent metric will be ignored and won't be created in the database.
    \item \texttt{test\_metric\_value\_bulk\_update\_rejects\_non\_list}: Test that the format of the payload to request bulk updates is correct. This ensures there are less issues downstream which can result in potential missed data.
    \item \texttt{test\_metric\_value\_bulk\_update\_success}: We also test the base case that the bulk update is triggered with the correct payload, and the expected metric values are updated.
  \end{enumerate}
\subsection{User Management Module}
  Tests for this module validates that our business logic determining the amount of functionality a user can do within the system by their role in the system is working as expected.
  \begin{enumerate}
    \item \texttt{test\_list\_users\_superadmin/user}: Only superadmin should have the ability to list all the users in the system. Admin and user roles, as well as unauthenticated users should get a forbidden response when calling the endpoint.
    \item \texttt{test\_superadmin/admin/user\_update\_role}: For security, only superadmin should be able to update other user's roles. Both admin and user shouldn't be able to, and receive forbidden responses if they call the endpoint.
    \item \texttt{test\_superadmin/admin/user\_edit\_user\_profile}: As usual, superadmin have the permission to edit user information for all. Admin and users can only edit their own profile and receive forbidden responses if they call the endpoint to edit someone else's profile.
    \item \texttt{test\_superadmin/admin/user\_edit\_domain}: We don't want anyone editing a domain so editing domain is restricted to superadmins, who can edit all the domains, and admins, who can edit their own domain. Normal users will get a forbidden response.
  \end{enumerate}

\subsection{Automated Metrics Module}
  Tests for this module ensure the extraction of the associated automated metrics from Github's API, git\_stats, and scc are successfully, and correctly stored in the database.
  An example set of tests for the response from Github's API:
  \begin{enumerate}
    \item \texttt{test\_extract\_repo\_info\_valid\_url}: Tests that we extract the repository name and owner from github's api correctly.
    \item \texttt{test\_extract\_repo\_info\_strips\_git\_suffix}: Tests that we remove suffix from github's response.
  \end{enumerate}

  We also test that during the analysis and processing of automated metrics, the system displays to the user the correct status of current operations.
  An example such as:
  \begin{enumerate}
    \item \texttt{test\_analyze\_repo\_task\_success\_updates\_metrics\_and\_marks\_success}: User should have feedback when the automated metrics are all successfully extracted and stored in the database for the associated library.
    \item \texttt{test\_analyze\_repo\_task\_sets\_running\_status\_early}: We should inform the user that something is happening in the system, even if we haven't actively started extracting from the library.
    \item \texttt{test\_analyze\_repo\_task\_exception\_marks\_failed\_and\_raises}: If there is a failure, user feedback should be given and errors should be handled smoothly.
  \end{enumerate}

\subsection{Comparison Module}
  Tests for this module consists of testing if the comparison of libraries within a domain is working as expected. It includes testing the AHP calculations and using different weights for categories.
  We also test if the module can gracefully handle error and provide user feedback when problems occur.
    \begin{enumerate}
      \item \texttt{test\_ahp\_calculations\_success}: The base case, when we use the AHP calculations it should process the passed in data in the manner we expect.
      \item \texttt{test\_ahp\_calculations\_not\_found}: Calling the comparison endpoint with an invalid domain id should return an error response, instead of failing.
      \item \texttt{test\_ahp\_calculations\_with\_weighted\_categories}: Using AHP calculations and adjusting the weight of categories, we should see the expected results based on the rules set.
      \item \texttt{test\_domain\_comparison\_returns\_metrics\_and\_rows\_with\_values\_and\_evidence}: We have a multitude of data gathered for each library in a domain. We should be able to retrieve all of them successfully with their corresponding evidence (for the automated ones) in order to be displayed.
    \end{enumerate}
  \subsection{Database Persistence Module}
    The tests for this are in relation to the persistence of our models in the database. They will include the validation and serialization of the data for the following database models:
    \begin{multicols}{2}
      \begin{enumerate}
        \item users
        \item domain
        \item libraries
        \item library metric values
      \end{enumerate}
    \end{multicols}
...
These are the user tasks referenced in provided to the user. Each question is rated on a 1–5
Likert scale (1 = Very Easy, 5 = Very Difficult) based on the difficulty it is to complete.

\begin{enumerate}
  \item Create an account and login.
  \item Create a new Domain.
  \item Add Metrics to the Domain you created.
  \item Add two libraries to your domain.
  \item Fill out the manually added metrics for the libraries you added.
  \item Export the data you gathered as a CSV for your domain.
  \item Visualize and export raw metric values.
  \item Visualize and Export your AHP calculations.
\end{enumerate}
  \newpage{}
