...
\subsection{Comparison Module}
  ...
  \subsection{Database Persistence Module}
    ...
    \begin{enumerate}
      \item test\_domain\_str\_returns\_name: Tests if calling a domain object will return domain name instead of domain id.
      \item test\_get\_domain\_id\_returns\_string\_uuid: Tests that the get\_domain\_id function returns the uuid of the domain. 
      \item test\_defaults\_and\_blank\_fields: Tests the creation of a domain object with only required fields, and check if blank fields are populated with the expected values.
    \end{enumerate}
...
\begin{enumerate}
  \item \texttt{test\_library\_raw\_values\_appear}: The raw values of the metric values should appear for the associaited metrics across selected libraries.  
\end{enumerate} 
\section{References}
