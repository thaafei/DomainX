...
Dec 16, 2025 & Fei & Rev 0 Based on issues extracted from TA review and peer reviews. The list of related issues can be found in this \href{https://github.com/thaafei/DomainX/pull/307}{pull request}. \\
Mar 18, 2026 & Fei & Final Documentation changes, compiled in this \href{https://github.com/thaafei/DomainX/issues/497}{github issue} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{description}
  ...
  \item[\refstepcounter{acnum} \actheacnum \label{ac8}:] Additional export file formatting may be added (e.g. CSV, Excel)
  \item[\refstepcounter{acnum} \actheacnum \label{ac9}:] The user interface features, design and colours might be changed.
\end{description}
...
\item [\refstepcounter{mnum} \mthemnum \label{mAppUI}:] Application UI Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mUserMgm}:] User Management Module
\item [\refstepcounter{mnum} \mthemnum \label{mAutoMetrics}:] Automated Metrics
...
between requirements and modules is listed in Table~\ref{tb_fr_modules}.

On a boarder scope all the functional requirements are covered by one module.
In cases where multiple modules covers a single functional requirement, it is due to the required functionality requiring multiple layers of system processes.
For example, FR5 "Admin can create domains, using a unique domain name and optionally a short description.", the modules needed to fulfill the requirement is \mref{mDomainsPage} and \mref{mUserRole}.
Since the domains' page is visible to all users, but only Admin users can create domains, we require the two modules to work simultaneously to achieve this requirement.
...
\item[Implemented By:] Web Browser (i.e. Chrome, Firefox, Safari, Edge)
\end{description}
...
\item[Services:]  Handles and validates user inputs of domain/package/metric data from user-inputted data and allows editing of automated data. Provides error checking for each metric based on their requirements.
...
\subsubsection{ User Management Module (\mref{mUserMgm})}
\begin{description}
\item[Secrets:] The data structures and algorithms used to store and validate users access level within the system.
\item[Services:] Provides user role and capabilities related to the role, which allows other modules to restrict features and display based on the role of the user.
\item[Implemented By:]  \projname
\item[Type of Module:] ADT
\end{description}

\subsubsection{ Automated Metrics Module (\mref{mAutoMetrics})}
\begin{description}
...
\item[Services:] Defines available comparison methods based on user request and interfaces with the graphing module where appropriate.
\item[Implemented By:] \projname
\item[Type of Module:] Abstract Object
\end{description}
...
\item[Services:] Manages application state and serves as the central communication hub for the system. First point of contact for changes to the business logic.
\item[Implemented By:] \href{https://www.djangoproject.com/}{Django}
\item[Type of Module:] Abstract Object
\end{description}
...
\begin{description}
\item[Secrets:] The algorithms and external libraries required to graph and visualize user data.
...
\item[Services:] Fetches metrics and metadata from external repositories (e.g., GitHub, GitLab). Handles the communications with the external library and provides an abstract layer between the raw external API interface and the system interface.
...
\midrule
FR1 & \mref{mUserMgm}\\
FR2 & \mref{mUserAuth}, \mref{mUserMgm}\\
FR3 & \mref{mUserAuth}\\
FR4 & \mref{mUserMgm}\\
FR5 & \mref{mDomainsPage}, \mref{mUserMgm} \\
FR6 & \mref{mDomainsPage} \\
FR7 & \mref{mDataEdit}, \mref{mDB}, \mref{mExport} \\
FR8 & \mref{mDataEdit} \\
FR9 & \mref{mAutoMetrics} \\
FR10 & \mref{mAutoMetrics}, \mref{mRepoAPI}\\
...
Table \ref{tb_fr_modules} shows the traceability between the functional requirements and the modules.
Note that \mref{mAppUI}, \mref{mSysApi} and \mref{mLog} is applicable to all, since they facilitate the information flow and user interaction for all the listed functional requirements and all functional requirements interactions will be logged.
...
PR-SC2 & \mref{mUserAuth}, \mref{mUserMgm}\\
PR-SC3 & \mref{mUserMgm}, \mref{mDataEdit}\\
PR-PA1 & \mref{mAutoMetrics}\\
...
PR-SE2 & \mref{mUserAuth}, \mref{mUserMgm} \\
...
SR-AC1 & \mref{mUserMgm}, \mref{mDomainsPage}\\
SR-AC2 & \mref{mUserMgm}\\
SR-AC3 & \mref{mUserMgm}\\
SR-AC4 & \mref{mUserMgm}\\
SR-AC5 & \mref{mAppUI}\\
SR-AC6 & \mref{mRepoAPI}, \mref{mDB}\\
SR-INT1 & \mref{mDataEdit}\\
SR-INT2 & \mref{mDB}\\
SR-INT3 & \mref{mDataEdit}, \mref{mDB}\\
SR-INT4 & \mref{mAppUI}, \mref{mDataEdit}, \mref{mAutoMetrics}, \mref{mDB}\\
SR-INT5 & \mref{mDB}, \mref{mLog}\\
SR-INT6 & \mref{mDataEdit}\\
SR-INT7 & \mref{mExport}, \mref{mSysApi}, \mref{mAppUI}\\
SR-P1 & \mref{mDB}, \mref{mUserAuth}, \mref{mUserMgm}\\
SR-P2 & \mref{mDB}, \mref{mUserAuth}, \mref{mUserMgm}\\
SR-AU1 & \mref{mDB}, \mref{mLog}\\
SR-AU2 & \mref{mLog}\\
SR-AU3 & \mref{mLog}, \mref{mUserMgm}\\
SR-IM1 & \mref{mDB}\\
SR-IM2 & \mref{mAppUI}\\
...
Note that Productization (OE-PR*), Release (OE-RR*), Maintenance (MS-MR*), Supportability (MS-SR*), Adaptability (MS-AR*), Legal (CR-LR*) requirements are not considered, since they are out-of-scope for this document. \\
SR-IM4 is also listed as Not Relevant, as it relates to ensuring external libraries used during development are up-to-date, which is out-of-scope for this document.

More information about the nonfunctional requirements can be found in the \href{https://github.com/thaafei/DomainX/blob/main/docs/SRS/SRS.pdf}{SRS document}.
\begin{table}[H]
\centering
\begin{tabular}{p{0.2\textwidth} p{0.6\textwidth}}
\toprule
...
\acref{ac5} & \mref{mUserMgm}\\
\acref{ac6} & \mref{mRepoAPI}, \mref{mAutoMetrics}\\
\acref{ac7} & \mref{mGraphing}\\
\acref{ac8} & \mref{mExport}\\
\acref{ac9} & \mref{mAppUI}\\
\bottomrule
\end{tabular}
...
Note that \mref{mSysApi} is applicable to all the anticipated changes, since they cover the data flow for the system.
...
\\ \acref{ac5} touches on what the user can see, if additional roles are added, the user page UI must be updated (\mref{mUserMgm}). The configuration to store each user's preference and role will also be updated (\mref{mUserMgm}).
\\ To account for \acref{ac6}, the underlying API will need to be updated (\mref{mRepoAPI}), and the data retrieved from the new API might differ (\mref{mAutoMetrics}).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/usehiechy.png}
\caption{Use hierarchy among modules}
\label{FigUH}
\end{figure}

M\ref{mHH1} and M\ref{mHH2} are required for all functionality due to the project being a web application, they have been left out of the diagram as they are implied.
...
\label{FigUI6}
\end{figure}
Figure \ref{FigUI6} shows the main user information page that any users can see. This page allows users to set their basic information. Additional side columns are added based on the role access level.
...
1 & Jan 5 -- Jan 11 & Haniye & Implement \textbf{User Authentication Module (\mref{mUserAuth})} for secure login, registration, token management, and password encryption. \\ \cline{3-4}
  &  & Ghena & Implement \textbf{User Management Module (\mref{mUserMgm})} with Admin/Analyst/Viewer roles and middleware for route protection. \\ \cline{3-4}
  &  & Fei & Setup testing framework \\ \cline{3-4}
  &  & Awurama & Build User Page (\mref{mUserMgm}) \\ \hline
...
4 & Jan 26 -- Feb 1 & Haniye & Implement \textbf{Automated Metrics Module (\mref{mAutoMetrics})} with background job scheduler and automatic metric retrieval. \\ \cline{3-4}
