...
\begin{itemize}
  ...
  \item \textbf{LogConfig} ::= $\langle storeType:\{\texttt{FILE},\texttt{DB}\}, level:LogLevel \rangle$
\end{itemize}
...
\begin{itemize}
  \item transition:
  \begin{itemize}
    \item Formats the log entry with timestamp, level, message, and context.
    \item If \texttt{logConfig.storeType = FILE}, the log entry is persisted to a file under \texttt{logDir}.
    \item If \texttt{logConfig.storeType = DB}, the log entry is persisted via the Database Persistence Module (M17).
  \end{itemize}
