...
getRecentLogs & limit, [filter] & logsList & ReadError \\
...
\begin{itemize}
  ...
  \item \textbf{logStore}: target logging sink (e.g., file store or database table).
  \item \textbf{logDir}: directory/location for log storage if file-based.
  \item \textbf{net}: network interface used if logs are shipped to an external service.
\end{itemize}
...
\begin{itemize}
  \item transition: formats the log entry and appends it to the configured log store.
  \item output: \texttt{successFlag} = true if the entry is persisted.
  \item exception: \texttt{WriteError} if persistence fails.
\end{itemize}

\noindent \textbf{getRecentLogs}(limit, [filter]):
\begin{itemize}
  \item transition: none.
  \item output: \texttt{logsList} containing up to \texttt{limit} most recent entries matching \texttt{filter} if provided.
