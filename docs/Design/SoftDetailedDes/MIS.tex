...
Mar 19, 2026 & Fei & Final Documentation changes, compiled in this \href{https://github.com/thaafei/DomainX/issues/501}{github issue} \\
\bottomrule
\end{tabularx}
...
JWT & JSON Web Token, authoritzation tokens. \\
...
\subsection{MIS of Application UI Module (M3)}
  ...
  File Export Module (M13) (\ref{mis:FileExportModule})\\
  ...
  \midrule
  initAppShell & none & mountedRoot (object) & RenderError \\
  navigateTo & routeID& renderedView (object) & RouteNotFound \\
  renderComponent & componentID, props & renderedFragment (object) & RenderError \\
  handleUserAction & actionType, payload & dispatchResult (object) & ActionError \\
  showNotification & message, severity & successFlag (bool) & None \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \paragraph{State Variables}
  \begin{itemize}
    \item \textbf{currentRoute}: the active route (path).
  ...
  \paragraph{Environment Variables}
  \begin{itemize}
    \item \textbf{screen}: display surface used to render the user interface (provided via M2 [\ref{mis:Browser}]).
    \item \textbf{inputDevices}: user input sources (mouse, keyboard, touch) that generate UI events (via M2 [\ref{mis:Browser}]).
    \item \textbf{net}: network interface used to send and receive requests to external services (via M2 [\ref{mis:Browser}]).
    \item \textbf{clock}: system clock used for timeouts and session timing (via M2 [\ref{mis:Browser}]).
  \end{itemize}


  \paragraph{Assumptions}
  \begin{itemize}
    \item Executed within the Browser Module (\ref{mis:Browser}), which provides DOM rendering, event delivery, storage, and networking facilities.
    \item Network I/O occurs exclusively through the System API Gateway Module (\ref{mis:SystemAPIGateway}); backend endpoints are not embedded in the UI.
    \item Graphs are rendered via \ref{mis:GraphingModule}; file workflows are delegated to \ref{mis:FileExportModule}.
  \end{itemize}
  ...
  \noindent \textbf{navigateTo}(routeID):
  \begin{itemize}
  ...
  \begin{itemize}
    \item transition: may update \texttt{uiState}; may delegate to modules listed in uses section via callbacks.
  ...
  \begin{itemize}
    ...
    \item \texttt{bindGraphHandlers()} — wires chart events (M12 [\ref{mis:Graphing}]) to UI callbacks.
...
\subsection{MIS of Data Edit Module (M4)}
  ...
  User Management Module (M6) (\ref{mis:UserMgm})\\
  System API Gateway Module (M10) (\ref{mis:SystemAPIGateway})\\
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  addMetric & metricData & successFlag (bool) & SchemaError \\
  editMetric & metricID, newValue & successFlag (bool) & ValidationError \\
  deleteMetric & metricID & successFlag (bool)& NotFoundError \\
  undoLastEdit & -- & successFlag (bool)& StackEmptyError \\
  validateEdit & candidateChange & successFlag (bool) & ValidationError \\
  commitBatch & changeList & successFlag (bool) & NetworkError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \paragraph{Assumptions}
  \begin{itemize}
    \item The user is authenticated (M5 [\ref{mis:UserAuthentication}]) and authorized (M6 [\ref{mis:UserMgm}]) for the requested action.
    \item Executed within a browser-based React UI; form state and feedback are managed by M3 [\ref{mis:ApplicationUI}].
    \item Network I/O is performed through M10 [\ref{mis:SystemAPIGateway}] over HTTPS using JSON payloads conforming to M10 schemas.
  \end{itemize}
  ...
  \begin{itemize}
    \item transition: validate \texttt{metricData}; stage in \texttt{editBuffer}; on commit, send create request via M10 to M15 [\ref{mis:DatabasePersistenceModule}].
  ...
  \begin{itemize}
    ...
    \item output: \texttt{successFlag} = true if deletion is confirmed by M15 [\ref{mis:DatabasePersistenceModule}] (via M10 [\ref{mis:SystemAPIGateway}]).
  ...
  \begin{itemize}
    \item transition: send up to \texttt{MAX\_EDIT\_BATCH} staged changes via M10 [\ref{mis:SystemAPIGateway}]; clear successful items from \texttt{editBuffer}; push entries to \texttt{undoStack}.
    \item output: \texttt{successFlag} = true if all changes are acknowledged by M15 [\ref{mis:DatabasePersistenceModule}].
...
\subsection{MIS of User Authentication Module (M5)}
  ...
  System API Gateway Module (M10) (\ref{mis:SystemAPIGateway})\\
  User Management Module (M6) (\ref{mis:UserMgm})\\
  Database Persistence Module (M15) (\ref{mis:DatabasePersistenceModule})\\
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  sendInvite & email, role & inviteCode (JWT)& AuthError \\
  registerUser & email, password, inviteCode & authToken (JWT) & ValidationError \\
  loginUser & email, password & authToken (JWT) & AuthError \\
  logoutUser & authToken & successFlag (bool)& SessionError \\
  validateSession & authToken & successFlag (bool) & TokenError \\
  refreshToken & oldToken & newToken (JWT) & TokenExpiredError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \begin{itemize}
    \item \textbf{net}: network interface used to receive authentication requests
          and return responses via M10 [\ref{mis:SystemAPIGateway}].
  ...
  \begin{itemize}
    \item transition: validate \texttt{inviteCode}; if valid, create a new user in M15 [\ref{mis:DatabasePersistenceModule}] with a hashed password and the role associated with the invite.
...
\subsection{MIS of User Management Module (M6)}
  \label{mis:UserMgm}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  assignRole & userID, roleName & successFlag (bool)& InvalidRoleError \\
  getUserRole & userID & roleName (str) & NotFoundError \\
  verifyAccess & userID, actionType & successFlag (bool) & PermissionError \\
  updateRolePermissions & roleName, newPermissions & successFlag (bool) & ConfigError \\
  listAllRoles & -- & roleList (list[str]) & None \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \paragraph{Environment Variables}
  \begin{itemize}
    \item \textbf{net}: network interface used to request and return authorization decisions via M10 [\ref{mis:SystemAPIGateway}].
  ...
  \paragraph{Assumptions}
  \begin{itemize}
    \item All users are authenticated before authorization checks are performed (via M5 [\ref{mis:UserAuthentication}]).
    \item Each \texttt{actionType} requested by other modules is mapped to a permission category understood by this module.
    \item Role and permission data is stored persistently via M15 [\ref{mis:DatabasePersistenceModule}] and accessed through M10 [\ref{mis:SystemAPIGateway}].
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \texttt{syncWithBackend()} – persists role/permission updates via M10 [\ref{mis:SystemAPIGateway}].
  \end{itemize}


\subsection{MIS of Automated Metrics Module (M7)}
  ...
  \subsubsection{Uses}
  Repository API Module (M14) (\ref{mis:RepositoryAPIModule})\\
  System API Gateway Module (M10) (\ref{mis:SystemAPIGateway})\\
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  fetchMetricsFromRepo & packageID & metricsData (object) & APIError \\
  validateMetrics & metricsData & successFlag (bool) & ValidationError \\
  storeValidatedMetrics & metricsData & successFlag (bool) & WriteError \\
  enqueueMetricsFetch & packageID & successFlag (bool) & QueueError \\
  processMetricsQueue & None & successFlag (bool) & WorkerError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \paragraph{Environment Variables}
  \begin{itemize}
    \item \textbf{net}: network interface used to contact external APIs (via M15) and submit stored metric results (via M10 [\ref{mis:SystemAPIGateway}]).
  ...
  \begin{itemize}
    \item transition: \item transition: submits validated metric records to persistent storage via M10 [\ref{mis:SystemAPIGateway}].
  ...
  \begin{itemize}
    \item transition: dequeue pending repositories and fetch supported metrics
          via M15 [\ref{mis:DatabasePersistenceModule}]; validate and store results via M10 [\ref{mis:SystemAPIGateway}].
...
\subsection{MIS of Domains Page Module (M8)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  fetchAllDomains & filterOptions & domainList (list[str]) & DatabaseError \\
  displayDomainDetails & domainID & renderedView (object) & DataNotFoundError \\
  renderDomainMetrics & domainID, metricType & chartObject (object) & RenderError \\
  searchDomains & searchTerm & filteredList (list[str]) & None \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \paragraph{Assumptions}
  \begin{itemize}
    \item Domain and metric data is available through the System API Gateway (M10 [\ref{mis:SystemAPIGateway}]).
    \item Users have appropriate permissions (via M6 [\ref{mis:UserMgm}]) to view domain data.
  ...
  \begin{itemize}
    \item transition: retrieves stored metric data for the domain via M10 [\ref{mis:SystemAPIGateway}] and delegates visualization to M12.
...
\subsection{MIS of Comparison Module (M9)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  selectComparisonMethod & methodName & successFlag (bool) & InvalidMethodError \\
  computeComparison & domainSet, metricWeights & comparisonTable (object) & ComputationError \\
  fetchComparisonData & domainSet & metricsData (object) & DatabaseError \\
  visualizeComparison & comparisonTable, chartType & chartObject (object) & RenderError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \begin{itemize}
    ...
    \item \textbf{net}: network interface used to request comparison data via M10 [\ref{mis:SystemAPIGateway}].
  \end{itemize}

  \paragraph{Assumptions}
  \begin{itemize}
    \item Required domain/metric data for comparisons is accessible through M10 [\ref{mis:SystemAPIGateway}].
  ...
  \begin{itemize}
    \item transition: requests required metric data via  [\ref{mis:SystemAPIGateway}] and computes comparison results using the selected method; may delegate scoring/weighting to M11 [\ref{mis:RankingAlgorithm}].  
  ...
  \begin{itemize}
    ...
    \item exception: \texttt{DataAccessError} if data cannot be retrieved through M10 [\ref{mis:SystemAPIGateway}].
  \end{itemize}
...
\subsection{MIS of System API Gateway Module (M10)}
  ...
  \subsubsection{Module}
  Implements the backend REST API layer that governs communication between the frontend and the server application, and mediates access to persisted data via M15 [\ref{mis:DatabasePersistenceModule}].
  ...
  Database Persistence Module (M15) (\ref{mis:DatabasePersistenceModule})\\
  User Authentication Module (M5)  (\ref{mis:UserAuthentication})\\
  Automated Metrics Module (M7) (\ref{mis:AutomatedMetrics})\\
  Repository API Module (M14) (\ref{mis:RepositoryAPIModule})\\
  User Management Module (M6) (\ref{mis:UserMgm})
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  handleHTTPRequest & request & response (object) & NetworkError \\
  authenticate & request & userContext (object) & AuthError \\
  authorize & userContext, action & successFlag (bool) & PermissionError \\
  validatePayload & endpoint, payload & successFlag (bool) & ValidationError \\
  dispatchToModule & endpoint, payload, userContext & serviceOutput (object) & RouteError \\
  buildResponse & serviceOutput & response (object) & SerializeError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \begin{itemize}
    ...
    \item Frontend tokens are validated by M5 [\ref{mis:UserAuthentication}].  
  ...
  \begin{itemize}
    \item transition: extracts credentials (e.g., session/JWT token) from \texttt{request} and validates them via M5 [\ref{mis:UserAuthentication}].
  ...
  \begin{itemize}
    \item transition: checks whether \texttt{userContext} is permitted to perform \texttt{action} using M6 [\ref{mis:UserMgm}].
...
\subsection{MIS of Ranking Algorithm Module (M11)}
  ...
  \subsubsection{Uses}
  Comparison Module (M9) (\ref{mis:Comparison})\\
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  computeRankScores & metricsTable, userWeights & rankTable (dict) & InvalidMetricError \\
  applyAHPWeights & criteriaMatrix & weightedMatrix (dict) & MatrixError \\
  normalizeScores & rankTable & normalizedTable (dict) & NormalizationError \\
  getTopPackages & normalizedTable, limit & rankedList (list[object]) & None \\
  \bottomrule
  \end{tabularx}
  \end{center}
...
\subsection{MIS of Graphing Module (M12)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  generateGraph & metricData, graphType, config & graphImage (object) & GraphError \\
  updateGraphStyle & styleParams & successFlag (bool) & StyleError \\
  exportGraph & graphImage, format, fileName & exportPath (str) & ExportError \\
  renderComparisonPlot & domainMetrics, metricType & comparisonImage (object) & DataError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \begin{itemize}
    ...
    \item \textbf{net}: network interface used to return visualization outputs via M10 [\ref{mis:SystemAPIGateway}].
  \end{itemize}
...
\subsection{MIS of File Export Module (M13)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  prepareExportData & queryParams, dataType & exportData (object) & DataError \\
  exportToCSV & exportData, fileName & filePath (str) & WriteError \\
  exportToExcel & exportData, fileName & filePath (str) & WriteError \\
  getExportSummary & filePath & summaryDict (dict) & None \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \begin{itemize}
    \item transition: requests the required records via M10 [\ref{mis:SystemAPIGateway}] using \texttt{queryParams} and \texttt{dataType}.
...
\subsection{MIS of Repository API Module (M14)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  setAuthToken & token & successFlag (bool) & AuthError \\
  fetchRepoMetadata & packageID & repoData (object) & APIError \\
  fetchRepoMetrics & packageID, metricList & metricsData (object) & APIError \\
  handleRateLimit & responseHeaders & delaySeconds (float) & RateLimitError \\
  \bottomrule
  \end{tabularx}
  \end{center}
...
\subsection{MIS of Database Persistence Module (M15)}
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  createEntity & entityType, entityData & entityID (UUID) & WriteError \\
  readEntity & entityType, query & entitySet (object) & NotFoundError \\
  updateEntity & entityType, entityID, patch & successFlag (bool) & WriteError \\
  deleteEntity & entityType, entityID & successFlag (bool) & DeleteError \\
  executeQuery & querySpec & queryResult (object) & QueryError \\
  \bottomrule
  \end{tabularx}
  \end{center}
...
\subsection{MIS of Logging Module (M16)}
  ...
  \subsubsection{Uses}
  System API Gateway Module (M10)  (\ref{mis:SystemAPIGateway})\\
  Database Persistence Module (M15) (\emph{optional; only if} \texttt{storeType = DB}) (\ref{mis:DatabasePersistenceModule})\\
  ...
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions}\\
  \midrule
  initLogger & logConfig & successFlag (bool)& ConfigError \\
  logEvent & message, level, context & successFlag (bool)& WriteError \\
  getRecentLogs & limit, [filter:LogFilter] & list(LogEntry) & ReadError \\
  archiveLogs & None & successFlag (bool)& ArchiveError \\
  \bottomrule
  \end{tabularx}
  \end{center}
  ...
  \texttt{logDir}. Database persistence via M15 [\ref{mis:DatabasePersistenceModule}] is supported by this MIS but is not required for the peer implementation.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item If \texttt{logConfig.storeType = DB}, persist \texttt{entry} via the Database Persistence Module (M15).
    \end{itemize}
