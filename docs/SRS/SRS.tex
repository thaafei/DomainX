...
\midrule
December 28, 2025 & Fei & Update based on TA and Peer Review \href{https://github.com/thaafei/DomainX/pull/254}{feedback issues}\\
March 30, 2026 & Fei & Final documentation update contained listed in this \href{https://github.com/thaafei/DomainX/issues/520}{Github issue}.\\
\bottomrule
\end{tabularx}
\end{table}
...
The reason for building Domain Comparison Tool can be found in the sections below.
\subsection{User Business}
The “methodology to assess the state of best practice for Domain X” paper \cite{MethodologyPaper}, outlines a series of systematic steps to assess the state of best practice for a particular domain.
...
\subsubsection{G1: Centralized Domain Data Location}\label{goal-central-data}
...
\subsubsection{G2: Automated Data Collection}\label{goal-auto-data}
...
\subsubsection{G3: Interactive Data Table}\label{goal-data-table}
...
\subsubsection{G4: Automated Analytic Hierarchy Process}\label{goal-auto-ahp}
...
\subsubsection{G5: Data Visualization and Download}\label{goal-visual}
...
\subsubsection{G6: Collaboration and Access Control}\label{goal-collab}
...
\subsection{Client}
The main users for the tool, and the individuals who requested it.
...
\textbf{Related Goals}: On top of the implied goal of having a completed tool, also concerned with \nameref{goal-collab} and \nameref{goal-central-data}
...
\textbf{Related Goals}: \nameref{goal-auto-data}, \nameref{goal-data-table}, \nameref{goal-auto-ahp}, and \nameref{goal-visual}, 
...
Additional stakeholders for the development or maintenance of the tool.
...
\textbf{Related Goals:} Since the goals are structured from the perspective of the user, there are no explicitly listed goals for the development team, other than the completion of the project.
...
\textbf{Related Goals}: \nameref{goal-data-table}, \nameref{goal-visual} and \nameref{goal-collab} 

\subsection{Hands-On Users of the Project}
Broader users of the tool.
\subsubsection{Dr. Spencer Smith}
See \hyperref[smith]{Dr. Spencer Smith}.
\subsubsection{Research Subteam}
See \hyperref[research_team]{Research Subteam}.
...
\subsection{Personas}
Below are personas to help visualize the users of the tool.
...
The priority level for each user groups.
\textbf{Key Users}:
\begin{itemize}
...
The following contains how the development team engages the user during development.
...
Users responsible for maintaining the tool after the initial development
...
Constraints on the eventual design of the tool. 
\subsection{Solution Constraints}
Constraints on a particular way the tool should solve and behave.
\begin{itemize}
...
Excel integration is essential. Since Excel was the format utilized by the previous ad hoc tool, enabling planned import and export functionality will facilitate a smooth transition and allow for the potential backfilling of historical data.
...
Below are the naming conventions and terminology that will be used throughout the video.
...
Boundaries of the current environment and business requirements to understand the scope of the product are written below.
\subsection{The Current Situation}
The following are the steps as stated from the methodology paper \cite{MethodologyPaper}:
\begin{enumerate}
...
The following table shows all business events to which the work responds. Business events are happenings in the real world that affect the work. They also happen because it is time for the work to do something.
\begin{table}[H]
\centering
...
This section contains a specification of the business objects, entities and classes that relate to the product.
\subsection{Business Data Model}
The following is an entity diagram of the key entities of the product, and how they interact.
\begin{figure}[H]
  \centering
...
Visualization of the scope we decided for the tool.
...
\begin{itemize}
  ...
  \item \textbf{AHP Validation:} The system will not include functionality to check the accuracy of the AHP calculations since external libraries will be used to implement the process itself.
\end{itemize}
...
Summary of all the interfaces between the product and other systems.\\

\begin{longtable}{|p{2cm}|p{2cm}|p{2cm}|p{7cm}|}
\caption{Product Use Case} \\ \hline
\toprule
...
\bottomrule
\endlastfoot
Invite New Users  (\nameref{goal-collab}) & Admin & Researcher & Admin can input a new user email. New user will receive an email with a link that will allow them to setup an account, password, and username. \\  \hline
Domain Creation \& Setup  (\nameref{goal-central-data}) & Admin & Researcher & Create new Domain (must validate unique name). Optional: Add description to the domain. \\  \hline
Publish Domain  (\nameref{goal-central-data}) & Researcher & General Users & Once the data collection is finished, allow anyone to see it (General Users). \\  \hline
Automation Library Data (\nameref{goal-auto-data})  & Researcher & System & Auto-fill fields (e.g., creation date, commit date) upon entering a public URL. Check for and flag missing data that was expected to be collected. \\  \hline
Manually Input/Update Data  (\nameref{goal-data-table}) & Researcher & System & Add missing data through the data table. Missing data must be highlighted in red. Only allow user to input right format (e.g., numbers, text). Maintain an audit trail of changes. \\  \hline
Ranking AHP (\nameref{goal-auto-ahp})  & Researcher & System & System will run the integrated AHP tool using input quality scores and expert pairwise comparisons to automatically calculate the final package rankings. Run process automatically when the table is filled and updated. \\  \hline
Visualize \& Export Results (\nameref{goal-visual}) & General User / Researcher / Admin & System & Allow selection of domain, libraries, and metrics to 2D graphs. Allow download of collected data (JSON/Excel). Allow download of graphs (PNG/LaTeX). \\  \hline
\end{longtable}
...
What our tool must do, it's features and behaviours are listed below.   
\begin{enumerate}[label=FR\arabic*]
  ...
  \\ \textbf{Rationale}: In case the user forgets their credentials
  \\ \textbf{Fit Criterion}: User receives password reset email if the email exist, and the next login will use the new password instead of the old one to allow user access.
  \item Admin can create domains, using a unique domain name and optionally a short description.
  \\ \textbf{Rationale}: To prevent all users to create domains, only domains that are verified by the product owner (Dr. Spencer Smith) will be able to create domains.
  \\ \textbf{Fit Criterion}: Admin users can create domains, while normal users cannot.
  \item Admin can publish a domain, once research is completed, for anyone to see and download data collected.
  \\ \textbf{Rationale}: To allow for domains to be in different stages, and for open access once completed.
  \\ \textbf{Fit Criterion}: Unpublished domains cannot be seen by users. Published domains can be seen by users.
  ...
  \\ \textbf{Rationale}: Researchers must be able to continuously update and analyze domain data throughout the research lifecycle, including before a domain is finalized or published.
  \\ \textbf{Fit Criterion}: Researchers can create, edit, and delete libraries and associated metric data within domains they have access to, and can download datasets and generated graphs whether the domain is published or unpublished.
  ...
  \\ \textbf{Rationale}: Input validation is required to ensure data correctness, consistency, and reliability for downstream analysis and visualization.
  \\ \textbf{Fit Criterion}: Invalid inputs are rejected at submission time with clear error messages, and only data that meets defined validation rules is saved to the system.
  \item The system must allow an Admin to trigger the automated metric data. The system must report detailed errors for any records that fail validation, such as feature names not being in the scope or wrong data type for a column.
  \\ \textbf{Rationale}: Reduces manual data entry effort and minimizes human error when working with multiple libraries.
  \\ \textbf{Fit Criterion}: Admins can enter a valid Github URL relating to a library, and the system successfully imports valid metrics while displaying a detailed error report for each invalid row and column.
  ...
  \\ \textbf{Rationale}: Automatable repository metric data should be automatically extracted.
  \\ \textbf{Fit Criterion}: With a valid Github URL, system populates the corresponding rows and columns with the collected metrics, and displays errors for irretrievable data.
  \item The system must display a complete list of all metrics and their definitions for a selected domain.
  \\ \textbf{Rationale}: Users should be able to clearly see what metrics are and what does metrics mean.
  \\ \textbf{Fit Criterion}: When a domain is selected, the system displays all associated metrics along with their definitions in a readable format.
  ...
  \\ \textbf{Rationale}: System should provide a more transparent and automated method to run the AHP between libraries to reduce manual human labour.
  \\ \textbf{Fit Criterion}: System computes ranking using AHP and displays final results in a readable and traceable way.
  \item Users must be able to visualize differences in libraries using comparative graphs.
  \\ \textbf{Rationale}: Provides better user readability and quick view of the collected data.
  \\ \textbf{Fit Criterion}: Users can select a domain, one or more libraries, relevant metrics, and a graph type, and the system generates the corresponding comparative visualization.
  ...
  \\ \textbf{Rationale}: Providing multiple export formats allows users to reuse data for analysis, reporting, and academic publication.
  \\ \textbf{Fit Criterion}: Users can successfully download selected datasets in JSON, Excel, or LaTeX formats, and the downloaded files contain the correct data.
  \item Users must be able to download any generated graph, with the option to save it as a PNG file.
  \\ \textbf{Rationale}: Users may need to include visualizations in reports, presentations, or publications.
  \\ \textbf{Fit Criterion}: Users can download any generated graph as a PNG file, and the downloaded image accurately reflects the on-screen visualization.
\end{enumerate}

\section{Look and Feel Requirements}
Requirements for the appearance of the tool.
...
\begin{enumerate}[label=LF-AR\arabic*]
  ...
  \\ \textbf{Fit Criterion}: Primary action buttons use a consistent color and style across the application and are visually more prominent than secondary actions. Users can easily identify buttons within 10 seconds.
  \item The system must be fully responsive and function correctly on standard desktop monitors and laptops.
  \\ \textbf{Fit Criterion}: The application functions correctly at common desktop and laptop screen sizes without overlapping or hidden UI elements.
  ...
  \\ \textbf{Fit Criterion}: Tables with five or more rows use alternating row colors, and all table text remains readable at default browser zoom.
  ...
  \\ \textbf{Fit Criterion}: The dashboard displays active domains and tasks on a single screen without requiring scrolling under normal usage.
  ...
  \\ \textbf{Fit Criterion}: All required fields are clearly marked, and validation feedback appears immediately when input is accepted or rejected.
  ...
  \\ \textbf{Fit Criterion}: Error messages clearly state the issue and suggest a corrective action in one or two short sentences.
\end{enumerate}
...
\begin{enumerate}[label=LF-SR\arabic*]
  ...
  \\ \textbf{Fit Criterion}: All icons used for navigation, actions, and status indicators follow the same visual style (e.g., all outline or all filled) across the application.
  \item Graphs must use minimalist design to prioritize data readability. Axes and labels must be clear and legible.
  \\ \textbf{Fit Criterion}: All graphs include labeled x- and y-axes with text readable at 100\% browser zoom, and labels are legible without zooming on a standard desktop display.
  ...
  \\ \textbf{Fit Criterion}: The application uses no more than three font sizes for titles, body text, and captions, and these sizes are applied consistently across all pages.
\end{enumerate}

\section{Usability and Humanity Requirements}
Requirements for the usability, and accessibility of the tool.
...
\begin{enumerate}[label=UH-EU\arabic*]
  ...
  \\ \textbf{Fit Criterion}: The primary navigation menu is visible on all screens, and common actions (viewing data, downloading reports) can be completed in three or fewer user actions.
  ...
  \\ \textbf{Fit Criterion}: All metric input fields display a tooltip on hover that specifies the expected data type and format.
  ...
  \\ \textbf{Fit Criterion}: All system error messages avoid technical error codes and include a brief explanation and a clear corrective action.
\end{enumerate}
...
\item All words referring to research entities (Domain, Library, Metric) must be used consistently across the interface and documentation.
  \\ \textbf{Fit Criterion}: All references to Domain, Library, and Metric use the same terms across the UI and documentation, with no alternate naming.
\end{enumerate}
...
\item All system-generated error messages will be clear, use non-technical language, and suggest a simple course of corrective action.
  \\ \textbf{Fit Criterion}: All system-generated error messages use non-technical language and suggest at least one corrective action.
\end{enumerate}
...
\begin{enumerate}[label=UH-AR\arabic*]
  ...
  \\ \textbf{Fit Criterion}: All critical status indicators and visualizations include text labels or patterns in addition to color.
  ...
  \\ \textbf{Fit Criterion}: All pages remain readable and functional when browser font size is increased up to 200\%.
\end{enumerate}

\section{Performance Requirements}
Requirements for the performance of the tool itself.
...
\begin{enumerate}[label=PR-SL\arabic*]
  ...
  \\ \textbf{Fit Criterion}: The AHP ranking calculation completes in under 10 seconds for a standard domain.
\end{enumerate}
\subsection{Safety-Critical Requirements}
\begin{enumerate}[label=PR-SC\arabic*]
  \item The system will encrypt all user passwords.
  \\ \textbf{Fit Criterion}: All user passwords are stored in encrypted (hashed) form and never in plaintext.
  ...
  \\ \textbf{Fit Criterion}: After a predefined number of consecutive failed login attempts, the account is locked and the user is notified via email.
  \item The system will ensure that only researchers modify data.
  \\ \textbf{Fit Criterion}: Only users with Researcher or Admin roles can modify domain data. Other users are blocked from write operations.
...
\begin{enumerate}[label=PR-PA\arabic*]
  ...
  \\ \textbf{Fit Criterion}: All automatically retrieved data matches the API used with transparent processing, if any.
  \end{enumerate}
...
\begin{enumerate}[label=PR-RFT\arabic*]
  ...
  \\ \textbf{Fit Criterion}: Failures in external services (e.g., GitHub API) do not crash the application and display a user-facing error message with at least one next step.
  ...
  \\ \textbf{Fit Criterion}: If a data update fails mid-operation, no partial changes are saved to the database.
  ...
  \\ \textbf{Fit Criterion}: Database backups exist and allow restoration of system data within one hour.
\end{enumerate}
...
\begin{enumerate}[label=PR-CR\arabic*]
  ...
  \\ \textbf{Fit Criterion}: The system supports up to 20 concurrent active users without noticeable performance degradation.
  \item The database must be able to store at least 70 different study domains.
  \\ \textbf{Fit Criterion}: The database can store at least 70 domains without errors.
  ...
  \\ \textbf{Fit Criterion}: Increasing database size does not cause connection failures, timeouts, or data corruption.
\end{enumerate}
...
\begin{enumerate}[label=PR-SE\arabic*]
  ...
  \\ \textbf{Fit Criterion}: Data storage and analysis/visualization components are implemented as separate modules.
  ...
  \\ \textbf{Fit Criterion}: New user roles can be added through configuration or extension without modifying core application logic.
\end{enumerate}
...
\begin{enumerate}[label=PR-LR\arabic*]
  ...
  \\ \textbf{Fit Criterion}: The system operates without mandatory maintenance interruptions for a period of two years under normal usage.
  ...
  \\ \textbf{Fit Criterion}: Database scaling can be completed through infrastructure configuration changes without modifying application source code.
\end{enumerate}

\section{Operational and Environmental Requirements}
Requirements for the operating environment and operational environment the tool will run in.
\subsection{Expected Physical Environment}
\textbf{Fit Criterion}: The system runs successfully on standard desktop or laptop hardware using a modern web browser and internet connection.
...
\textbf{Fit Criterion}: The application functions correctly on the latest two to three major versions of Chrome, Firefox, and Edge with an active internet connection.
...
\begin{enumerate}[label=OE-IA\arabic*]
  ...
  \\ \textbf{Fit Criterion}: System calls external APIs immediately when the scan is triggered, and the system must be able to receive JSON over HTTPS.
  \item Database (MySQL): The backend must store scores, rankings, and evidence collected from repositories.
\\ \textbf{Fit Criterion}: All scores, rankings, and evidence are stored and retrievable from the database within 10 seconds.
...
\\ \textbf{Fit Criterion}: Charts are rendered correctly using backend-provided data using visualization libraries.
...
\\ \textbf{Fit Criterion}: All external communications use HTTPS and authenticated API access where required.
\end{enumerate}
...
Requirements for the maintenance and support of the tool once the initial development has left.
...
Requirements on the security in relation to data, such as the privacy and integrity of it.
...
\begin{enumerate}[label=SR-AC\arabic*]
  ...
  \item User roles (Viewer / Contributor) must be assigned and updated correctly at sign-up or by an admin so that permissions always reflect the intended access level. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}

  \item The system must display clear error messages for login failures (e.g., invalid credentials, network errors) and provide a secure way for users to recover or reset their account credentials. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}
...
\item The system must detect simultaneous edits and either block one save or notify the users to resolve the conflict. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}
\item When automated data updates conflict with a user’s manual edits (if any are updated automatically), the system must warn the user or request confirmation before replacing existing data. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}
...
\item Publishing visualizations must not occur at the same time as data edits or automated refreshes. The system must either block publishing or enforce downtime until updates are complete. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}
\item If an export (e.g., CSV, PNG) fails or produces a corrupted file, the tool must alert the user and allow them to retry the download. \textit{(From FMEA table, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}.)}
\end{enumerate}
...
The below table contains the open issues at the start of the project.
\begin{table}[!htbp]
\centering
...
These products may serve as inspiration or integration points (e.g., Excel export) but cannot replace the custom automation and analysis required by the NNL Assessment Tool.
...
\href{https://github.com/thaafei/DomainX/blob/main/docs/ProblemStatementAndGoals/ProblemStatement.pdf}{Problem Statement}, POC Plan, \href{https://github.com/thaafei/DomainX/blob/main/docs/DevelopmentPlan/DevelopmentPlan.pdf}{Development Plan}. \\
\hline
Requirements &
Gather and formalize system requirements. &
Stakeholder interviews, drafting of SRS and Hazard Analysis. &
\href{https://github.com/thaafei/DomainX/blob/main/docs/SRS/SRS.pdf}{SRS Document}, \href{https://github.com/thaafei/DomainX/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}. \\
\hline
Design &
Establish system architecture and interfaces. &
UI mockups, data flow diagrams, schema design. &
Design Documents, \href{https://github.com/thaafei/DomainX/blob/main/docs/Design/SoftArchitecture/MG.pdf}{MG} and \href{https://github.com/thaafei/DomainX/blob/main/docs/Design/SoftDetailedDes/MIS.pdf}{MIS}. \\
\hline
Implementation &
...
\href{https://github.com/thaafei/DomainX/blob/main/docs/VnVPlan/VnVPlan.pdf}{V\&V Plan}, \href{https://github.com/thaafei/DomainX/blob/main/docs/VnVReport/VnVReport.pdf}{V\&V Report}. \\
\hline
Deployment &
Deliver hosted solution. &
Deploy tool, prepare documentation, conduct final demo. &
\href{https://domainx.cas.mcmaster.ca/}{Final Hosted Tool}, Presentation, \href{https://github.com/thaafei/DomainX/blob/main/docs/ReflectAndTrace/ReflectAndTrace.pdf}{Final Reflect and Trace Report}. \\
\bottomrule
\end{tabularx}
\end{table}
