...
repository URLs). Under the current configuration, TensorFlow achieved the
highest overall score, followed by Catboost, OpenCV,
PyTorch, and Hugging Face Transformers. Beyond the
top-ranked package, many libraries were more tightly
clustered, with the strongest differentiation arising from
installability and correctness and verifiability.
...
\subsection{Interpreting TensorFlow's lead}
TensorFlow's position as the top-ranked package is driven
primarily by a small set of differentiating categories
rather than broad dominance across all assessed qualities.
In particular, TensorFlow received a substantially larger
value in correctness and verifiability than the other
shortlisted packages, and this had a strong effect on the
overall ranking under the current configuration. TensorFlow
also shows comparatively strong repository-based metric
values, which further separate it from much of the
shortlist.

At the same time, the results do not suggest that
TensorFlow is uniquely superior in every quality dimension.
Several assessed categories are relatively uniform across
most packages, meaning that TensorFlow's overall lead is
best interpreted as the result of exceptional performance in
a few highly differentiating areas rather than consistently
higher scores across the board. This is important because it
shows that the ranking should be read as an evidence-based
comparison under the current scoring and weighting setup,
rather than as a universal claim that one package is best in
all respects.
...
The current results suggest that the overall ranking is
driven primarily by a small subset of differentiating
categories, rather than by broad variation across all
qualities. In particular, installability and correctness and
verifiability contribute strongly to the separation between
packages, while several of the remaining surface-level
qualities are comparatively uniform across the shortlisted
set. This means that, under the current assessment
configuration, packages that perform strongly in the most
differentiating categories can rise substantially in the
overall ranking.
...
which quality dimensions currently contribute most to the
observed differences across packages. For maintainers, the
results highlight areas where visible software-engineering
practice can be strengthened and made more consistent.

Several recommendations follow from the current findings.
First, maintainers should improve installability by providing
clear installation instructions, dependency information, and
...
assurance practices. Third, maintainers should continue to
improve transparency through well-maintained public
artifacts, since these materials directly affect the
evidence available for state-of-practice assessment.
Finally, future refinements of the assessment process should
consider how to better differentiate categories that are
currently highly uniform across the shortlisted packages.
...
Similarly, categories that appear uniform in the current
results may reflect limitations of the available evidence or
of the applied scoring scheme rather than true equality
across packages.
...
The current results show that the overall ranking is shaped
primarily by a small subset of differentiating categories,
especially installability and correctness and verifiability,
while several other quality categories are comparatively
uniform across the shortlisted packages. TensorFlow emerged
as the top-ranked package under the current configuration,
with the remaining packages more tightly clustered below it.

Future work includes refining categories that are currently weakly
differentiated, and extending the assessment with additional
...
final assessed set used throughout this paper. Packages that
appeared in intermediate tool views but were not retained in
...
produce the overall ranking reported in the paper. Under the
current configuration, the category weights are nearly
uniform, with installability assigned a slightly larger
priority than the remaining categories. The pairwise
comparison interface and resulting calculated priorities are
shown in Figure~\ref{fig:app-weights}. These materials are
included to support transparency and reproducibility of the
ranking configuration used in DomainX.

\begin{table}[H]
\centering
\caption{Final AHP category weights used in the current configuration.}
...
Installability & 7.7\% \\
Quality & 7.7\% \\
Correctness and Verifiability & 7.7\% \\
Surface Reliability & 7.7\% \\
Surface Robustness & 7.7\% \\
Surface Usability & 7.7\% \\
Reusability & 7.7\% \\
Visibility/Transparency & 7.7\% \\
Surface Understandability & 7.7\% \\
Raw Metrics (Measured via \texttt{git\_stats}) & 7.7\% \\
Raw Metrics (Measured via \texttt{scc}) & 7.7\% \\
Repo Metrics (Measured via GitHub) & 7.7\% \\
\bottomrule
\end{tabular}
\end{table}
