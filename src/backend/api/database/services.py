...
class RepoAnalyzer:
    ...
    def _clone_repo(self):
        """Clones the GitHub repository into a local directory."""
    ...
    def _get_git_metrics(self):
        """Uses local git commands to get metrics requiring a local repo (e.g., total commits/branches)."""
        metrics = {}
        try:
            # Commit Count
    ...
    def _analyze_repo(self):
        ...
        # Filter combined results to only include numerical values for target metrics
        final_results = {
    ...
    def run_analysis_and_get_data(self):
        """
        The main execution flow: clone -> analyze -> return data -> cleanup.
        """
        try:
